...
`onloadedmetadata` で `video.currentTime = trim_start` をセット。再生中は毎フレーム `relativeTime >= obj.start + (trim_end ?? file_duration) - (trim_start ?? 0)` を監視して `<video>` をポーズし Canvas 描画をスキップする。シーク時も `(trim_start ?? 0) + offset` に currentTime を設定する。
...
Canvas 上でオブジェクトをマウスドラッグして `x`, `y` を変更できる。

- ヒットテスト: `pointerdown` 時に Z 順最前面（配列末尾）から順にバウンディングボックス判定
- 音声オブジェクトはヒットテスト対象外
- `setPointerCapture` で Canvas 外へのポインター移動もトラッキング
- ドラッグ開始時に `projectStore.selectScene` / `selectObject` を呼んでオブジェクトを選択
- カーソル: ホバー時 `grab`、ドラッグ中 `grabbing`
- ドラッグ中の座標変換: `projX = offsetX / (scale × displayScale)`
...
`scene.duration` が明示設定されている場合はそれを使う。未設定の場合は `PreviewCanvas` 内の `computeEffectiveSceneLen(scene)` がオブジェクトの終了時刻最大値を probe キャッシュを参照して計算する。

- probe 未完了のシーンは `Number.MAX_SAFE_INTEGER` を返し、「そのシーンに留まる」フォールバック動作とする
- `computeTotalDuration()` は各シーンの有効長の合計（`MAX_SAFE_INTEGER` は 0 として加算しない）

`getCurrentSceneEffective(time)` は probe ベースのシーン長を使ってシーンインデックスと相対時間を返す（`sceneUtils.ts` の `getCurrentScene` は `scene.duration ?? 0` しか参照しないため、マルチシーンかつ明示 `duration` なしの場合に使用しない）。

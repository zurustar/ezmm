...
interface Entry {
  name: string;
  // ES2015+ 以降、Record（オブジェクト）は string キーの挿入順を保持する。
  // Rust 側は indexmap::IndexMap で明示的に順序保持（型レベルで保証）。
  // 本アプリは挿入順保持に依存するため、実装時は ES2015+ 環境前提。
...
interface ValidationResult {
  errors: ValidationIssue[];     // バッチ実行をブロック
...
interface ValidationIssue {
  ...
  entry_name?: string;             // 対象エントリ名（該当する場合）
}

// VariableValue の厳密定義
type VideoVariable   = { file: string; trim_start?: number; trim_end?: number };
type ImageVariable   = { file: string };
type AudioVariable   = { file: string };
type TextVariable    = { text: string };
type VariableValue = VideoVariable | ImageVariable | AudioVariable | TextVariable;
```
...
特記なき限り、すべての struct / enum に共通で `#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]` を適用する（IPC 送信での `Clone`、ログ出力での `Debug`、スナップショットテストでの `PartialEq` を想定）。以降の例示では記述の簡潔化のため `Serialize, Deserialize` のみ記載している。

**例外**: Tauri イベントペイロード struct（`BatchProgressPayload` / `BatchEntryDonePayload` / `BatchEntryErrorPayload` / `BatchDonePayload`）は `emit()` 専用で受信側では使わないため `#[derive(Clone, Serialize)]` のみとする（`Deserialize` / `PartialEq` は不要）。
...
pub struct Entry {
    pub name: String,
    // IndexMap: 挿入順を保持（BTreeMap だとキーがソートされユーザー指定順が失われる）
    pub variables: indexmap::IndexMap<String, VariableValue>,
}
...
- 映像オブジェクトの実効長（= ファイル長 − trim_start − trim_end）がシーン長を決定する一要素
- シーン長 = シーン内オブジェクトの終了時刻の最大値

**循環参照の回避:** 以下のいずれかに該当するシーンはシーン長が定義不能になるためバリデーションエラーとする:
- 映像オブジェクトがなく、かつ `duration > 0` のオブジェクトが1つも存在せず（画像・テキスト・音声すべて `duration: 0.0`）、かつ `scene.duration` も未指定
- `loop: loop` の音声オブジェクトは `duration: 0.0` の場合「シーン終端まで」を意味するが、シーン終端は自分自身で決まる（循環）
...
`VideoVariable` / `ImageVariable` / `AudioVariable` はいずれも `{ file: string }` を含む同一形状。識別はバリデーション・レンダリング処理において「そのオブジェクト ID に対応するオブジェクトの `type` フィールド」を参照して行う。

| 層 | 定義 |
|---|------|
| TypeScript | 4 型の union (`VideoVariable` / `ImageVariable` / `AudioVariable` / `TextVariable`) |
| Rust | 2 variant の `#[serde(untagged)]` enum (`Media` / `Text`) |

JSON / YAML 上の表現は両者で完全に一致する（`{file: "...", trim_start?: ..., trim_end?: ...}` または `{text: "..."}`）。

---

## エントリ `name` の文字制約

OS共通のファイル名禁止文字を排除する方針（日本語・Unicodeを許容）:
- **禁止文字**: `/` `\` `:` `*` `?` `"` `<` `>` `|` および制御文字
- **空文字列禁止**
- **プロジェクト内でユニーク**
- これにより `田中` `鈴木_CFO` 等のファイル名が使用可能

---

## `variable: true` とデフォルト値の共存

`variable: true` のオブジェクトに YAML上で `file` や `text` を記述した場合、それをデフォルト値として扱う。エントリの `variables` に対応するキーが存在すれば上書き、存在しなければデフォルト値を使用。両方ない場合はバリデーションエラー。

---

## 空プロジェクト

シーン0件・エントリ0件でも有効なプロジェクトとして保存・読み込み可能。バッチ実行時のみ「シーンが存在しません」エラーで弾く。
...
```rust
// 将来の実装例（v1 → v2 マイグレーション）
#[derive(Debug, thiserror::Error)]
#[error("unsupported project version: {0}")]
pub struct UnsupportedVersionError(pub u32);

fn parse_v1(raw: serde_yml::Value) -> Result<Project, serde_yml::Error> {
    serde_yml::from_value(raw)
}

<script lang="ts">
	...
	import BatchProgressDialog from '../components/BatchProgressDialog.svelte';
	...
	// Show batch progress dialog when running or done
	let showBatchDialog = $derived(
		batchStore.status === 'running' ||
		batchStore.status === 'cancelling' ||
		batchStore.status === 'done'
	);

	// Show recent files on startup if available
	onMount(async () => {
		// Wait a tick for settingsStore to be populated by layout
	...
	function handleRequestNew() {
		withUnsavedCheck(newProject);
	}

	function handleRequestOpen() {
		withUnsavedCheck(() => openProject());
	}

	// Unsaved dialog callbacks
	...
	// ---- Keyboard shortcuts ----
	let isTextInput = (el: EventTarget | null) => {
		if (!el) return false;
	...
	function handleKeydown(e: KeyboardEvent) {
		...
		// Keys that require no text input focus
		...
		if (e.key === 'Delete' || e.key === 'Backspace') {
			if (projectStore.selectedObjectId && projectStore.selectedSceneId) {
				...
				projectStore.selectObject(null);
			} else if (projectStore.selectedEntryName) {
				const name = projectStore.selectedEntryName;
				projectStore.updateProject((p) => { p.entries = p.entries.filter((e) => e.name !== name); });
				projectStore.selectEntry(null);
			}
			return;
		}
		...
		if (mod && e.key === 'd') {
			...
			if (projectStore.selectedObjectId && projectStore.selectedSceneId) {
				...
				projectStore.updateProject((p) => {
					...
					if (idx < 0) return;
					const copy = { ...p.entries[idx], name: `${name}_copy`, variables: { ...p.entries[idx].variables } };
					p.entries.splice(idx + 1, 0, copy);
				});
			}
			return;
		}
		// Arrow keys: frame/5s seek
	...
	let isBatchBusy = $derived(batchStore.status === 'running' || batchStore.status === 'cancelling');
</script>
...
<!-- Dialogs / Modals -->
{#if showBatchDialog}
	<BatchProgressDialog onClose={() => batchStore.onCancelled()} />
{/if}

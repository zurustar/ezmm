...
fn main() {
    tauri::Builder::default()
        .setup(|app| {
            ...
            // バンドルパスが存在しない場合はシステム PATH にフォールバック（開発環境用）
            let bundled_ffmpeg = resource_dir.join("ffmpeg").join(ffmpeg_exe);
            let bundled_ffprobe = resource_dir.join("ffmpeg").join(ffprobe_exe);
            let ffmpeg_path = if bundled_ffmpeg.exists() { bundled_ffmpeg } else { PathBuf::from(ffmpeg_exe) };
            let ffprobe_path = if bundled_ffprobe.exists() { bundled_ffprobe } else { PathBuf::from(ffprobe_exe) };
